...
import { updateTeam } from "@/lib/server/room-service";
...
export async function POST(request: Request, { params }: RoomRouteProps) {
  try {
    ...
    const payload = await parseRequestBody(request, teamUpdateSchema);
    const snapshot = await updateTeam(code.toUpperCase(), payload.playerId, payload.team);
